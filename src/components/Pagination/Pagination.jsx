...
export const Pagination = ({ pagination }) => {
  ...
  const paginationCurrent =
    totalProducts === limit ? totalProducts : width < totalProducts ? width : width - limit + (totalProducts % limit);
  ...
  return (
    <div className={s.pagination}>
      ...
      <div className={s.arrows}>
        <Link className={`${s.left} ${prevPageURL ? "" : s.disabled}`} to={prevPageURL}>
        ...
        <p className={s.info}>
          <span>{paginationCurrent.toString()}</span> из <span>{totalProducts.toString()}</span>
        </p>
        <Link className={`${s.right}  ${nextPageURL ? "" : s.disabled}`} to={nextPageURL}>
          <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path
